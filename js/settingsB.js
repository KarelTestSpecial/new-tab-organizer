document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Data Management (View B) ---
    window.handleExport = () => {
        chrome.storage.sync.get(['panelsState_B', 'settings'], (data) => {
            if (chrome.runtime.lastError) {
                console.error(chrome.runtime.lastError);
                alert('Error exporting data for View B.');
                return;
            }
            const exportData = {
                panelsState_B: data.panelsState_B || [],
            ...
            a.href = url;
            a.download = `startpage-backup-B-${new Date().toISOString().slice(0, 10)}.json`;
    ...
    importFileInput.addEventListener('change', (event) => {
        ...
        reader.onload = (e) => {
            try {
                ...
                // Check for the correct panel state key for View B
                if (data && typeof data.panelsState_B !== 'undefined' && typeof data.settings !== 'undefined') {
                    if (confirm('Are you sure you want to import this data? Your current layout and settings for THIS VIEW will be overwritten.')) {
                        ...
                        chrome.storage.sync.set(dataToImport, () => {
                            if (chrome.runtime.lastError) {
                                console.error(chrome.runtime.lastError);
                                alert('Error importing data for View B.');
                            } else {
                                alert('Data for View B imported successfully! The page will now reload.');
                                location.reload();
                            }
                        });
                    }
                } else {
                    alert('Invalid backup file for View B. Make sure you are using a "backup-B" file.');
                }
            } catch (error) {
    ...
    document.getElementById('import-all-bookmarks-btn').addEventListener('click', () => {
        ...
        chrome.bookmarks.getTree((tree) => {
            ...
            findFolders(tree[0]);
            // Use the correct state for View B
            chrome.storage.sync.get('panelsState_B', (data) => {
                const currentPanels = data.panelsState_B || [];
                ...
                if (newPanelsAdded > 0) {
                    // Save to the correct state for View B
                    chrome.storage.sync.set({ panelsState_B: currentPanels }, () => {
                        alert(`${newPanelsAdded} new bookmark panels have been added to View B. The page will now reload.`);
                        location.reload();
                    });
                } else {
